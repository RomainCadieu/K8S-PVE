...
resource "proxmox_virtual_environment_vm" "etcd" {
    ...
    initialization {
        ip_config {
            ipv4 {
                address =   "${var.ETCD_IP_PREFIX == "dhcp" ? "dhcp" : "${var.ETCD_IP_PREFIX}${var.ETCD_IP_PREFIX_24 + count.index + 1}${var.ETCD_MASK}"}"
                gateway =   var.ETCD_GW
            }
        }
    ...
    connection {
      ...
      host     = "${var.ETCD_IP_PREFIX == "dhcp" ? "dhcp" : "${var.ETCD_IP_PREFIX}${var.ETCD_IP_PREFIX_24 + count.index + 1}"}"
    }
    ...
    provisioner "remote-exec" {
      inline = [
        "sudo mkdir /etc/salt",
        "sudo su -c 'echo -e role: etcd >> /etc/salt/grains'",
      ]
    }
    provisioner "file" {
