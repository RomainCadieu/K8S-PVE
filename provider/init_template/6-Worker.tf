...
resource "proxmox_virtual_environment_vm" "worker" {
    ...
    depends_on = [null_resource.templatization, proxmox_virtual_environment_vm.edge01[0],proxmox_virtual_environment_vm.master[0]]
    count               =   var.WORKER_NUMBER_OF_VM
    name                =   format("Worker%02d", count.index + 1)
    description         =   format("Worker %02s, managed by Terraform",count.index + 1)
    tags                =   ["terraform", "Debian12", "worker", "primary"]
    node_name           =   var.PROXMOX_VE_DEFAULT_NODE
    vm_id               =   format("%2s%02s",var.WORKER_ID_PREFIX, count.index + 1)
    on_boot           = false
    started           = true
    ...
    initialization {
        ip_config {
            ipv4 {
                address =   "${var.WORKER_IP_PREFIX == "dhcp" ? "dhcp" : "${var.WORKER_IP_PREFIX}${var.WORKER_IP_PREFIX_24 + count.index + 1}${var.WORKER_MASK}"}"
                gateway =   var.WORKER_GW
            }
        }
    ...
    connection {
      ...
      host     = "${var.WORKER_IP_PREFIX == "dhcp" ? "dhcp" : "${var.WORKER_IP_PREFIX}${var.WORKER_IP_PREFIX_24 + count.index + 1}"}"
    }
    ...
    provisioner "remote-exec" {
      inline = [
        "sudo mkdir /etc/salt",
        "sudo su -c 'echo -e role: worker >> /etc/salt/grains'",
      ]
    }
    provisioner "file" {
