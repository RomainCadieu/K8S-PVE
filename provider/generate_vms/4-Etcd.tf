...
resource "proxmox_virtual_environment_vm" "etcd" {
  depends_on = [proxmox_virtual_environment_vm.edge01[0], proxmox_virtual_environment_vm.edge[0]]
  count               =   var.Etcd.Count
  name                =   format("Etcd%02d", count.index + 1)
  description         =   format("Etcd%02s, managed by Terraform",count.index + 1)
  tags                =   ["Terraform", "Debian12", "Etcd"]
  node_name           =   var.Proxmox.Node
  vm_id               =   format("%2s%02s",var.Etcd.Id_prefix, count.index + 1)
  on_boot           = false
  started           = true
  ...
  initialization {
    ip_config {
      ipv4 {
        address =   "${var.Etcd.Ip_prefix == "dhcp" ? "dhcp" : "${var.Etcd.Ip_prefix}${var.Etcd.Ip_suffix + count.index + 1}${var.Etcd.Mask}"}"
        gateway =   var.LAN_GW
      }
    }
    ip_config {
      ipv4 {
        address     =   "${ var.WAN_IP_temp_prefix}${var.WAN_IP_temp_suffix + var.Edge.Count + 1 + count.index }${var.VyOS.LAN_Mask}"
      }
    }
    user_account {
      ...
      password      = random_password.etcd_password[count.index].result
      username      = random_string.etcd_username[count.index].result
    }
    datastore_id    = var.Proxmox.Datastore
  }
  connection {
    type     = "ssh"
    user     = random_string.etcd_username[count.index].result
    password = random_password.etcd_password[count.index].result
    private_key = "${file(var.Template.SSH_local_file)}"
    host     = "${ var.WAN_IP_temp_prefix}${var.WAN_IP_temp_suffix + var.Edge.Count + 1 + count.index }"
  }
  ...
  provisioner "file" {
    content     = tls_private_key.etcd_vm_key[count.index].private_key_openssh
...
resource "random_string" "etcd_username" {
  count             = var.Etcd.Count
  length            = 16
  special           = false
}
resource "random_password" "etcd_password" {
  count             = var.Etcd.Count
  length            = 16
  override_special  = "_%@"
  special           = true
}
resource "tls_private_key" "etcd_vm_key" {
  count             = var.Etcd.Count
  algorithm         = "RSA"
  rsa_bits          = 2048
}