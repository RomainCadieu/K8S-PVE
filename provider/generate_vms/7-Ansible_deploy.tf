locals {
  tf_VIP_IP = "${var.Edge.Ip_prefix}${var.Edge.Ip_suffix + 0}"
  Aedge_ipv4_address = flatten([
    for server, servers in proxmox_virtual_environment_vm.edge01.*.name : [
      ...
      v if length(regexall("^(lo|docker|veth|eth1).*", proxmox_virtual_environment_vm.edge01[server].network_interface_names[k])) == 0
    ]
  ])
  Bedge_ipv4_address = flatten([
    for server, servers in proxmox_virtual_environment_vm.edge.*.name : [
      ...
      v if length(regexall("^(lo|docker|veth|eth1).*", proxmox_virtual_environment_vm.edge[server].network_interface_names[k])) == 0
    ]
  ])
  Cedge_ipv4_address = concat(local.Aedge_ipv4_address, local.Bedge_ipv4_address)
  edge_ipv4_address = [for region in local.Cedge_ipv4_address : region if region != local.tf_VIP_IP]
  master_ipv4_address = flatten([
    for server, servers in proxmox_virtual_environment_vm.master.*.name : [
      ...
      v if length(regexall("^(lo|docker|veth|eth1|vxlan.calico|calib).*", proxmox_virtual_environment_vm.master[server].network_interface_names[k])) == 0
    ]
  ])
  etcd_ipv4_address = flatten([
    for server, servers in proxmox_virtual_environment_vm.etcd.*.name : [
      ...
      v if length(regexall("^(lo|docker|veth|eth1).*", proxmox_virtual_environment_vm.etcd[server].network_interface_names[k])) == 0
    ]
  ])
  worker_ipv4_address = flatten([
    for server, servers in proxmox_virtual_environment_vm.worker.*.name : [
      ...
      v if length(regexall("^(lo|docker|veth|eth1).*", proxmox_virtual_environment_vm.worker[server].network_interface_names[k])) == 0
    ]
  ])
}
  ...
  content = templatefile("provider/templates/kube_inventory.tmpl",
    {
      ...
      no_init_master_ip   = slice(local.master_ipv4_address,1,var.Master.Count),
      no_init_master_user = slice(random_string.master_username.*.result,1,var.Master.Count),
  ...
  content = templatefile("provider/templates/tf_ansible_vars_file.tmpl",
    {
      ...
      master_ip           = local.master_ipv4_address,
      master_user         = random_string.master_username.*.result,
      no_init_master_dns  = slice(proxmox_virtual_environment_vm.master.*.name,1,var.Master.Count),
      no_init_master_ip   = slice(local.master_ipv4_address,1,var.Master.Count),
      no_init_master_user = slice(random_string.master_username.*.result,1,var.Master.Count),
